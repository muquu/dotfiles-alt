...
PROMPT="${RESET}${GREEN}${WINDOW:+"[$WINDOW]"}${RESET}$USERCOLOR${USER}$HOSTCOLOR@%m ${RESET}${WHITE}$ ${RESET}"
#PROMPT='${RESET}${GREEN}${WINDOW:+"[$WINDOW]"}${RESET}%{$fg[cyan]%}${USER}@%m ${RESET}${WHITE}$ ${RESET}'
RPROMPT='${RESET}${WHITE}[${PURPLE}%(5~,%-2~/.../%2~,%~)% ${WHITE}]${WINDOW:+"[$WINDOW]"} ${RESET}'
...
function zle-line-init zle-keymap-select {
  case $KEYMAP in
    vicmd)
    PROMPT="${RESET}${GREEN}${WINDOW:+"[$WINDOW]"}${RESET}%{$fg_bold[cyan]%}$USERCOLOR${USER}$HOSTCOLOR@%m ${RESET}${WHITE}$ ${RESET}"
    ;;
    main|viins)
    PROMPT="${RESET}${GREEN}${WINDOW:+"[$WINDOW]"}${RESET}$USERCOLOR${USER}$HOSTCOLOR@%m ${RESET}${WHITE}$ ${RESET}"
    ;;
...
cygwin*)
    # ここに設定
    [ -f ~/dotfiles/.zshrc.windows ] && source ~/dotfiles/.zshrc.windows
    ;;
esac
